...
<script>
  function generateProductGraph() {
    // Get the product data from the server
    $.ajax({
      url: "getProductData",
      method: 'GET',
      success: function(response) {
        var products = response.products; // Assuming the response contains an array of products with stock and sales data
        ...
        var stockData = products.map(function(product) {
          return product.stock;
        });

        var salesData = products.map(function(product) {
